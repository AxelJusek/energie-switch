...
public class VerbindungEnerGie implements ConnectionEnergie {
  ...
  private boolean empfangeStatus() throws IOException {
    ...
    if (!empfangen) {
      ...
      log.info("Es wurde die Statusmeldung: >" + byteArraytoString(this.encryptedStatus)
          + "< empfangen.");
   ...
   * 
   * @param arrayOfByte Bytes auf der Schnittstelle
  ...
  private byte[] getResponseForChallenge(byte task0, byte key2, byte key0, byte key4, byte key6,
      byte task2) {
    ...
    log.info("res[1:0]=((" + Integer.valueOf(t0).toString() + "^" + Integer.valueOf(k2).toString()
        + ")*" + Integer.valueOf(k0).toString() + ")^(" + Integer.valueOf(k6).toString() + " | ("
    ...
    log.info("res[1:0]=(" + Integer.valueOf(int1).toString() + "*" + Integer.valueOf(k0).toString()
        + ")^(" + Integer.valueOf(k6).toString() + " | (" + Integer.valueOf(k4).toString()
    ...
    int int5 = int2 ^ int4;
    log.info(
        "res[1:0]=" + Integer.valueOf(int5).toString() + "^" + Integer.valueOf(t2).toString() + "");
  ...
  public byte[] putPasswordInArray(String password) {
    ...
    if (8 >= password.length()) {
      ...
      throw new IllegalArgumentException(
          "Laenge des Passwortes ueberschreitet die Hoechstlaenge von 8 Zeichen.");
    }
    return this.key;
  }

  public Socket neueTCPVerbindung(String ipAddress, String port) {
    Integer portNr = Integer.parseInt(port);
    try {
      this.socket = new Socket(ipAddress, portNr);
    } catch (IOException e) {
  ...
  public void doseSchalten(Verbindungsdaten verbindungsDaten, String doseNr, String einschalten)
      throws IOException {
    VerbindungEnerGie vEnGen = verbindungAufbauen(verbindungsDaten);
  ...
  private static VerbindungEnerGie verbindungAufbauen(Verbindungsdaten verbindungsDaten)
      throws IOException {

    var vEnGen = new VerbindungEnerGie();
    byte[] key = vEnGen.putPasswordInArray(verbindungsDaten.getPasswd());

    log.info("Es werden die folgenden Parameter verwendet: \n" + "IP-Adresse '"
        + verbindungsDaten.getIpAddress() + "'\n" + "Port '" + verbindungsDaten.getPort() + "'\n"
